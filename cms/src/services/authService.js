...
export const AUTH_SERVICE = {
	async login ( data, dispatch )
	{
		try
		{
			const response = await postMethod('/user/auth/login', data);
			await timeDelay( 500 );
			return response;
		} catch ( error )
		{
	...
	async getProfile()
	{
		return await getMethod('/user/auth/profile?type=2');
	},

	async update ( data )
	{
		return await putMethod( `/user/auth/profile?type=2`, data );
	},
